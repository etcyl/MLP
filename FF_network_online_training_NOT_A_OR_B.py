# -*- coding: utf-8 -*-
"""
Created on Sat Jul 30 20:50:04 2016
...
class backpropagate(object):
    ...
    def train(self, inputs, targets, network):
        ...
        self.error = abs(targets[self.x][0] - out)
...
def writeCSV(zipped):
    csvout = csv.writer(open("test10.csv", "wb"))
    csvout.writerow(("Iteration", "Accuracy"))
    for i in range(len(zipped)):
        csvout.writerow((i, zipped[i]))
    
        
def trainAndPlot(inputs, targets, n, b, iters):
    i = []
    y = []
    count = 0
    ...
    plt.title('~ A | B training: average error versus training iterations')
    plt.plot(i, y)
